...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, indices):
        ...
        attention_scores = torch_scatter.scatter_softmax(attention_scores, indices, dim=0)  # [N, 1]
        x = x * attention_scores  # [N, C]
        pooled_data = torch_scatter.scatter_sum(x, indices, dim=0)  # [M, C]
        return pooled_data
...
class DynamicConv(nn.Module):
    ...
    def forward(self, x):
        # x: [N, C]
        kernels = self.kernel_generator(x)  # [N, out_dim * in_dim]
        kernels = kernels.view(-1, self.out_dim, self.in_dim)  # [N, out_dim, in_dim]
        x = torch.bmm(kernels, x.unsqueeze(-1)).squeeze(-1)  # [N, out_dim]
        return x
...
class MultiScaleFeatureExtractor(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.conv1 = nn.Conv1d(in_dim, out_dim // 2, kernel_size=1)
        self.conv2 = nn.Conv1d(in_dim, out_dim // 2, kernel_size=3, padding=1)
        self.bn = nn.BatchNorm1d(out_dim)

    def forward(self, x):
        # x: [N, C]
        x = x.unsqueeze(0).transpose(1, 2)  # [1, C, N]
        x1 = self.conv1(x)  # [1, out_dim//2, N]
        x2 = self.conv2(x)  # [1, out_dim//2, N]
        x = torch.cat([x1, x2], dim=1)  # [1, out_dim, N]
        x = self.bn(x).squeeze(0).transpose(0, 1)  # [N, out_dim]
        return x

class ptBEVnet(nn.Module):
    
    def __init__(self, BEV_net, grid_size, pt_model = 'pointnet', fea_dim = 3, pt_pooling = 'attention', kernal_size = 3,
                 out_pt_fea_dim = 64, max_pt_per_encode = 64, cluster_num = 4, pt_selection = 'farthest', fea_compre = None):
        super(ptBEVnet, self).__init__()
        assert pt_pooling in ['max','attention']
        ...
        if pt_model == 'pointnet':
            self.PPmodel = nn.Sequential(
                ...
                MultiScaleFeatureExtractor(fea_dim, 64),
                nn.BatchNorm1d(64),
                ...
                MultiScaleFeatureExtractor(256, out_pt_fea_dim)
            )
        ...
        if self.pt_pooling == 'attention':
            ...
            self.attention_pooling = AttentionPooling(out_pt_fea_dim)
    ...
    def forward(self, pt_fea, xy_ind, voxel_fea=None):
        ...
        if self.pt_pooling == 'max':
            pooled_data = self.attention_pooling(processed_cat_pt_fea, unq_inv)
        else: raise NotImplementedError
